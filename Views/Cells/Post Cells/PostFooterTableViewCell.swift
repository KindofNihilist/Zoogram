...
//        button.trailingAnchor.constraint(lessThanOrEqualTo: stackView.trailingAnchor, constant: -15).isActive = true
    ...
    public func configure(for post: UserPost) {
        var views = [UIView]()
        views.append(likesLabel)
        
        if !post.likeCount.isEmpty {
            if post.likeCount.count > 1 {
                likesLabel.text = "\(post.likeCount.count) likes"
            } else {
                likesLabel.text = "\(post.likeCount.count) like"
            }
        } else {
        ...
        if let caption = post.caption {
            if !caption.isEmpty {
                let attributedUsername = NSAttributedString(string: "\(post.owner.username) ", attributes: [NSAttributedString.Key.font: UIFont.boldSystemFont(ofSize: 14), NSAttributedString.Key.foregroundColor: UIColor.label])
        ...
        timeSincePostedLabel.text = "\(post.postedDate)"
...
//            likesLabel.topAnchor.constraint(equalTo: stackView.topAnchor, constant: 10),
